...
def test_t02_zero_results_and_report(page: Page) -> None:
    ...
    with page.expect_response(lambda r: r.url.endswith("/ui/report") and r.request.method == "POST") as resp_info:
    ...
    try:
        ...
        body = ""
    (artifacts_dir / "t02-report-response.txt").write_text(f"status={resp.status}\nbody={body}\n", encoding="utf-8")
